...
4. **Player Journey Sketch**: D0~D30+ 시간축으로 최소 4단계(입문/탐색/습관/장기)를 스케치한다. 각 단계에서 플레이어가 무엇을 경험하는지, 다음 단계로 끌어당기는 훅이 무엇인지 정의한다. **훅은 이벤트 나열이 아니라 심리 메커니즘이다**: "어떤 순간에, 어떤 감정/인식이 생겨서, 다음 단계로 넘어간다"를 한 문장으로 쓴다. "신규 콘텐츠 추가로 장기 리텐션 유지"처럼 변화만 나열하는 훅은 불완전하다.
...
훅 작성 원칙 — "무엇이 바뀌는가" 나열 금지. **[어떤 순간에] + [플레이어에게 어떤 인식/감정이 생겨서] + [다음 단계로 넘어간다]** 구조로 쓴다.
좋은 예: "D7 첫 오프라인 수입을 발견하는 순간, '내가 없어도 진행된다'는 인식이 생기면서 앱 열기가 의무에서 확인으로 바뀐다."
나쁜 예: "신규 콘텐츠 추가로 장기 리텐션 유지."
...
6. [ ] Player Journey가 입문~장기(최소 4단계)로 작성되고, 각 훅이 [순간]+[심리 변화]+[결과 행동] 구조로 쓰였는가 — 이벤트 나열이 아닌 심리 메커니즘
